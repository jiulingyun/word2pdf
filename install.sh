...
install(){
    ...
    cd /usr/local/
    last_version=$(curl -Ls "https://api.github.com/repos/jiulingyun/word2pdf/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    ...
    wget -N --no-check-certificate -O /usr/local/word2pdf-${last_version}.tar.gz https://github.com/jiulingyun/word2pdf/releases/download/${last_version}/${last_version}.tar.gz
    if [[ $? -ne 0 ]]; then
